...
// BATA BEGIN
unsigned int static DarkGravityWaveBTA(const CBlockIndex* pindexLast, const Consensus::Params& params) {
...
unsigned int GetNextWorkRequiredBTA(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    {
        return DarkGravityWaveBTA(pindexLast, params);
    }
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Dead lock protection will halve work every block spacing when no block for 2 * number of active algos * block spacing (Bata: every two minutes if no block for 10 minutes)
